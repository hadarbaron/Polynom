...
/**
 * The class uses gral and creates a diagram of the specific function of 0.2x ^ 4-1.5x ^ 3 + 3.0x ^ 2-x-5
It draws the extreme points and uses the calculation function in the polynomial to calculate the area under the X axis 
 * @author הדגי
 *
 */
public class polgrraph extends JFrame {
	public polgrraph() {
		...
		{	
			if ((p2.f(x)>0 && p2.f(x+0.1)<0)||(p2.f(x)<0&&p2.f(x+0.1)>0))//Find the approximation of extreme points with the derative
			{
				...
				System.out.println("the x of the extreme point is: "+i+ " the y is: "+p1.f(i));
				data1.add(i,p1.f(i));
			}
			double y=p1.f(x);
			data.add(x,y);
